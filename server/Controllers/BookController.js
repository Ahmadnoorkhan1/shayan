...
const gptSummary = async (data) => {
    const { prompt, contentDetails = {} } = data;
    ...
    // Safely build character prompt
    const characterNames = characters
      ? characters.split(",").map((name) => name.trim())
      : [];
    ...
    let question = `Write a compelling 600-word summary for a book titled "${prompt}" with the following details:
  ...
  The summary should:
  - Introduce the ${mainCharacter} structure${
      characterNames.length > 0
        ? `, specifically featuring ${characterNames.join(" and ")}`
        : ""
    }
  - Establish the ${setting} setting and atmosphere`;
    ...
    if (characterNames.length > 0)
      question += `\n- Ensure ${characterNames.join(
        " and "
      )} have meaningful roles in the story`;
  ...
  Make sure to return only the summary text, without any additional formatting or meta-text.`;
    ...
    // Handle AI response with timeout
    const timeout = new Promise((_, reject) =>
